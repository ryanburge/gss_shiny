...
small_gss <- na.omit(small_gss)


small_gss %>%  filter(mainline == 1 & year == 1972) %>% ggplot(aes(x= partyid, fill=factor(partyid)), color= factor(partyid)) + geom_bar() + theme(axis.text.x = element_text(angle = 90)) + scale_fill_brewer(palette="RdBu", direction=-1)