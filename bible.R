...
bible <- gss %>% select(year, bible, reltrad, partyid)
...
plot <- bible %>% group_by(year, reltrad) %>% 
  ...
  mutate(label = c("Theology")) %>% 
  ungroup(year, reltrad) %>% bind_rows(plot1)


lit$reltrad <- Recode(lit$reltrad, "1='Evangelical Protestants';
2='Mainline Protestants';
3='Black Protestants';
4='Catholic';
5='Jewish';
...
lit %>% ggplot(., aes(x=year, y=repub*100, color = reltrad, label = reltrad)) + 
  theme(legend.position="bottom") + theme(legend.title = element_blank()) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text=element_text(size=18, family="KerkisSans")) + labs(x= "Year", y = "% Republican ID", title = "The Political Ideology of Biblical Literalists ", caption = "Data from the GSS (1984-2016)")+
...
plot %>% ggplot(., aes(x=year, y=rep*100, color = label, label = label)) + geom_point(aes(group=label), size = 1.5)   +
  theme(legend.position="bottom") + theme(legend.title = element_blank()) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text=element_text(size=18, family="KerkisSans")) + labs(x= "Year", y = "% Literalists or % Republican ID", title = "How Has Partisanship and Religious Conservatism Change Over Time? ", caption = "Data from the GSS (1984-2016)")+
...
ggsave(file="PID_literal.png", type = "cairo-png", width = 15, height = 10)